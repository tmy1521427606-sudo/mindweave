...
const itemsSchema = {
  ...
  schema: {
    ...
    properties: {
      items: {
        type: "array",
        maxItems: GENERATION_BATCH_SIZE,
        items: {
          type: "object",
          required: [
            "candidateId", "title", "contentType", "region", "priority", "topics", "fact", "sourceView",
            "background", "development", "impact", "relevance", "concepts", "connections", "uncertainty",
            "action", "oneLineValue", "score",
          ],
          properties: {
            candidateId: { type: "string" },
            title: { type: "string" },
            contentType: { type: "string", enum: ["news", "learning"] },
            region: { type: "string", enum: ["国内", "海外", "全球"] },
            priority: { type: "string", enum: ["必读", "关注", "扩展"] },
            topics: { type: "array", minItems: 1, items: { type: "string" } },
            fact: { type: "string" },
            sourceView: { type: ["string", "null"] },
            background: { type: "string" },
            development: { type: "array", minItems: 2, items: { type: "string" } },
            impact: { type: "array", minItems: 1, items: { type: "object", required: ["audience", "text"], properties: { audience: { type: "string" }, text: { type: "string" } } } },
            relevance: { type: "string" },
            concepts: { type: "array", minItems: 1, items: { type: "object", required: ["name", "explanation"], properties: { name: { type: "string" }, explanation: { type: "string" } } } },
            connections: { type: "string" },
            uncertainty: { type: "string" },
            action: { type: "string" },
            oneLineValue: { type: "string" },
            score: {
              type: "object",
              required: Object.keys(SCORE_LIMITS),
              properties: Object.fromEntries(Object.entries(SCORE_LIMITS).map(([name, maximum]) => [name, { type: "integer", minimum: 0, maximum }])),
            },
          },
        },
      },
...
const INVALID_ITEM_REASONS = Object.freeze({
  requiredText: 0,
  invalidDate: 0,
  invalidEnum: 0,
  invalidStructure: 0,
...
export async function generateIssueContent({
  ...
  if (!Number.isInteger(minimumItems) || minimumItems < 5 || minimumItems > 10) throw new TypeError("invalid minimumItems");
  ...
  const remaining = Math.max(0, MAX_DAILY_ITEMS - existingItems.length);
  const selected = diverseCandidates(unique, remaining);
  if (existingItems.length + selected.length < minimumItems) throw contentError("insufficient_content", `可靠内容不足 ${minimumItems} 条`);
  ...
  const invalidItems = { ...INVALID_ITEM_REASONS };
  ...
  async function reportProgress() {
    ...
    if (partialItems.length >= 5) {
      try {
        ...
        initialIssue = buildIssue({
          date,
          items: partialItems.slice(0, 5),
          now,
          generation: { status: "time_limited", targetItems, failedBatches },
          minItems: 5,
        });
    ...
    await onProgress({ completedItems, totalItems: targetItems, failedBatches, invalidItems: { ...invalidItems }, repairedBatches, partialIssue, initialIssue });
  }
  ...
  async function worker() {
    while (nextBatch < batches.length) {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
          output = await doubao.chat({
            ...
            messages: [
              ...
              ...(attempt === 1 ? [{ role: "system", content: "上一次返回未通过格式校验。请严格按照 JSON Schema 为本批全部候选重新生成，不能省略字段，也不能改变 candidateId。" }] : []),
        ...
        const checked = validateBatchItems(output, byId, acceptedCandidateIds, date);
        for (const reason of checked.reasons) invalidItems[reason] += 1;
        generated = checked.items;
        if (checked.reasons.length === 0) {
          if (needsRepair) repairedBatches += 1;
          break;
        }
        needsRepair = true;
        if (attempt === 1) {
          failedBatches += 1;
          firstFailure ??= contentError("invalid_generated_content", "模型返回条目未通过格式校验");
        }
      }
      if (batchFailure) {
        failedBatches += 1;
      ...
      for (const item of generated) acceptedCandidateIds.add(item.candidateId);
      generated = generated.map(({ candidateId: _candidateId, ...item }) => item);
  ...
  if (items.length < minimumItems && firstFailure) throw firstFailure;
  const generation = failedBatches > 0
    ? { status: "partial_failures", targetItems: existingItems.length + selected.length, failedBatches }
    : undefined;
  return buildIssue({ date, items, now, generation, minItems: minimumItems });
}

function validateBatchItems(output, byId, acceptedCandidateIds, date) {
  if (!Array.isArray(output?.items) || output.items.length !== byId.size) {
    return { items: [], reasons: ["invalidStructure"] };
  }
  const items = [];
  const reasons = [];
  const seen = new Set();
  for (const raw of output.items) {
    ...
    if (!candidate) {
      reasons.push("unknownCandidate");
      continue;
    }
    if (acceptedCandidateIds.has(raw.candidateId) || seen.has(raw.candidateId)) {
      reasons.push("duplicateCandidate");
      continue;
    }
    try {
      const item = normalizeGeneratedItem(raw, candidate, date);
      validateItem(item, date);
      seen.add(raw.candidateId);
      items.push({ ...item, candidateId: raw.candidateId });
    } catch (error) {
      if (!(error instanceof DailyContentError)) throw error;
      reasons.push(validationReason(error));
    }
  }
...
function validationReason(error) {
  if (/日期/.test(error.message)) return "invalidDate";
  if (/枚举/.test(error.message)) return "invalidEnum";
  if (/缺少必要文字/.test(error.message)) return "requiredText";
  return "invalidStructure";
}

function buildIssue({ date, items, now, generation, minItems = 10 }) {
  const instant = now();
  ...
  return validateGeneratedIssue(issue, { minItems });
}

export function validateGeneratedIssue(issue, { minItems = 10, maxItems = MAX_DAILY_ITEMS } = {}) {
  ...
  if (issue.generation !== undefined && (!new Set(["time_limited", "partial_failures"]).has(issue.generation?.status)
    || !Number.isInteger(issue.generation.targetItems) || issue.generation.targetItems < issue.items.length || issue.generation.targetItems > MAX_DAILY_ITEMS
