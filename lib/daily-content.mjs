...
export async function generateIssueContent({
  ...
  async function worker() {
    while (nextBatch < batches.length) {
      ...
      const partialIssue = partialItems.length >= 10
        ? buildIssue({ date, items: partialItems, now, generation: { status: "time_limited", targetItems } })
        : null;
      onProgress({ completedItems, totalItems: targetItems, partialIssue });
    }
  }
  ...
  return buildIssue({ date, items, now });
}
...
export function validateGeneratedIssue(issue, { minItems = 10, maxItems = 30 } = {}) {
  ...
  if (issue.generation !== undefined && (issue.generation?.status !== "time_limited"
    ...
    throw contentError("invalid_generated_content", "限时生成标记无效");
  }
