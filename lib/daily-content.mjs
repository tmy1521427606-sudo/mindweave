...
export async function generateIssueContent({
  ...
  async function reportProgress() {
    ...
    await onProgress({ completedItems, totalItems: targetItems, failedBatches, partialIssue, initialIssue });
  }
  ...
  async function worker() {
    while (nextBatch < batches.length) {
      ...
      for (const raw of output.items) {
        ...
        if (!candidate || acceptedCandidateIds.has(raw.candidateId)) continue;
        try {
          const item = normalizeGeneratedItem(raw, candidate, date);
          validateItem(item, date);
          acceptedCandidateIds.add(raw.candidateId);
          generated.push(item);
        } catch (error) {
          if (!(error instanceof DailyContentError)) throw error;
        }
