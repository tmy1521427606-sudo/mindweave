...
export async function collectCandidates({ search, plan, excludedUrls = [], onProgress = () => {} }) {
  ...
  async function runWindow(window, windowName) {
    for (const entry of window.queries) {
      ...
      for (const result of Array.isArray(results) ? results : []) {
        ...
        } else if (excluded.has(normalized.candidate.url) || candidates.has(normalized.candidate.url)) {
