const SEARCH_GROUPS = Object.freeze([
  ...
  { topic: "数据 × Agent", query: "数据平台 AI Agent 官方 发布 GitHub" },
  { topic: "金融 × 科技", query: "金融科技 AI 官方 公告 监管 技术" },
  { topic: "Agent 开发与大模型", query: "AI agent framework model official release GitHub" },
  { topic: "模型发布与对比", query: "new AI model official release benchmark" },
  { topic: "知识图谱与多智能体", query: "knowledge graph ontology wiki multi-agent release research" },
]);
...
export function buildSearchPlan({ date, focusMore = [], focusLess = [], temporaryFocus = "", profile = {} }) {
  assertDate(date);
  ...
  return {
    ...
    profile,
    yesterday: { startDate: endDate, endDate, queries },
    expanded: { startDate: expandedStart, endDate, queries },
  };
}

export async function collectCandidates({ search, plan, onProgress = () => {} }) {
  ...
  async function runWindow(window, windowName) {
    for (const entry of window.queries) {
      ...
      for (const result of Array.isArray(results) ? results : []) {
        const normalized = normalizeCandidate(result, entry, windowName, window);
...
function normalizeCandidate(result, query, windowName, searchWindow) {
  ...
  if (!publishedDate) return { candidate: null, reason: "missingDate" };
  if (publishedDate < searchWindow.startDate || publishedDate > searchWindow.endDate) return { candidate: null, reason: "outsideWindow" };
  return { candidate: {
    url,
    title,
