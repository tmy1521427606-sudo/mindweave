  ...
  async function runWindow(window, windowName) {
    for (const entry of window.queries) {
      ...
      try {
        ...
        onProgress({ searchCalls, candidates: candidates.size, window: windowName });
        continue;
      }
      for (const result of Array.isArray(results) ? results : []) {
        const candidate = normalizeCandidate(result, entry, windowName, window);
...
function normalizeCandidate(result, query, windowName, searchWindow) {
  ...
  if (!publishedDate || publishedDate < searchWindow.startDate || publishedDate > searchWindow.endDate) return null;
  return {
    url,
    title,
