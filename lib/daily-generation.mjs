...
export function createDailyGenerationService({
  ...
  async function run(job) {
    ...
    const timeout = new Promise((_, reject) => {
      timeoutId = setTimer(() => {
        job.cancelled = true;
        reject(serviceError("generation_timeout", "日报生成超过三十分钟"));
      }, hardTimeoutMs);
    });
    try {
  ...
  async function execute(job) {
    ...
    const issue = await generateIssueContent({
      ...
      minimumItems: 5,
      now: clock,
      onProgress: async ({ completedItems, totalItems, failedBatches, invalidItems, repairedBatches, partialIssue, initialIssue }) => {
        ensureActive(job);
        job.completedItems = completedItems;
        job.targetItems = totalItems;
        job.failedBatches = failedBatches;
        job.invalidItems = { ...invalidItems };
        job.repairedBatches = repairedBatches;
  ...
  function snapshot(job) {
    const value = {
      ...
      repairedBatches: job.repairedBatches,
      invalidItems: { ...job.invalidItems },
