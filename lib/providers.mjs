...
export function createDoubaoClient({ apiKey, chatModel, embeddingModel, baseUrl = ARK_BASE_URL, fetchImpl = fetch } = {}) {
  ...
  const usesPromptSchema = chatModel.startsWith("glm-");
  const usesChatCompletions = chatModel.startsWith("ep-") || usesPromptSchema;
  const request = (path, body, timeoutMs) => postJson({
    fetchImpl,
    url: `${base}${path}`,
    provider: "Doubao",
  ...
  return {
    ...
    async chat({ messages, responseSchema } = {}) {
      ...
      const requestMessages = responseSchema !== undefined && usesPromptSchema
        ? [{ role: "system", content: schemaInstruction(responseSchema) }, ...messages]
        : messages;
      const body = usesChatCompletions
        ? { model: chatModel, messages: requestMessages }
        : { model: chatModel, input: messages, stream: false };
      if (responseSchema !== undefined && !usesPromptSchema) usesChatCompletions
        ? body.response_format = { type: "json_schema", json_schema: responseSchema }
        : body.text = { format: { type: "json_schema", ...responseSchema } };
      const payload = await request(usesChatCompletions ? "/chat/completions" : "/responses", body, CHAT_REQUEST_TIMEOUT_MS);
      const content = usesChatCompletions ? payload?.choices?.[0]?.message?.content : responsesText(payload);
      if (typeof content !== "string") throw invalidResponse("Doubao");
      try {
        return JSON.parse(content);
      } catch {
        throw invalidResponse("Doubao", "invalid_json");
      }
    },
...
function schemaInstruction(responseSchema) {
  return `只输出 JSON 对象，不要使用 Markdown 代码块或添加解释。输出必须符合以下 JSON Schema：${JSON.stringify(responseSchema.schema)}`;
}
...
async function postJson({ fetchImpl, url, provider, headers, body, timeoutMs = REQUEST_TIMEOUT_MS }) {
  let response;
  try {
...
function invalidResponse(provider, code = "invalid_response") {
